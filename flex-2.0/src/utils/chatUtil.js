...
export const getMessageCounts = (conversationSid) => {
  let agentMsgCount = 0;
  const flexState = _manager.store.getState().flex;
  const flexChatConversations = flexState.chat.conversations;
  console.log(PLUGIN_NAME, "Channels from Flex Redux", flexChatConversations);

  const convo = flexChatConversations[conversationSid];
